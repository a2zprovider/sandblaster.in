    ...
    <div>
                            ...
                            <div class="chksection">
                                ...
                                @foreach ($categories as $category)
                                    <div class="chk-row-sty d-flex align-items-center">
                                        ...
                                        <label for="cat-{{ $category->slug }}"
                                            class="text-truncate">{{ $category->title }}</label>
                                    </div>
                                @endforeach
                            </div>
                                    ...
                                    @foreach ($pf->filters as $f)
                                        <div class="chk-row-sty d-flex align-items-center">
                                            ...
                                            @if (@$r_query[$pf->slug] == $f->slug) 
                                              <input name="{{ $pf->slug }}" type="radio" checked id="{{ $pf->title . '-' . $f->slug }}" value="{{ $f->slug }}" onchange="debounceSubmit(this)">
                                            @else
                                              <input name="{{ $pf->slug }}" type="radio" id="{{ $pf->title . '-' . $f->slug }}" value="{{ $f->slug }}" onchange="debounceSubmit(this)">
                                            @endif
                                            </div>&nbsp;
                                            <label for="{{ $pf->title . '-' . $f->slug }}"
                                                class="text-truncate">{{ $f->title }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            @endforeach
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="btn-row pro-btn">
                                <a href="{{ route('page.list') }}" class="btn-1" style="text-decoration: none; color:#F0E5E1;"> Clear All</a>
                                @if($selected_category_details)
                                <button class="btn-2" style="margin-bottom: 15px;" id="text">{{$selected_category_details->title}}</button>
                                @endif
        ...
        Quotation</button> --}}
                                            <button type="button" class="custom-btn btn-3 get-but" data-bs-toggle="modal"
                                                ...
                                                <span><img src="{{ asset('images/img/quote.png') }}" alt="job image"
                                                        title="job image" style="width: 30px" /> Get Quotation
                                                </span></button>
                                        </div>
                                    ...
                                    <div class="col-lg-7">
                                        ...
                                        <div class="table-r">
                                            <div class="read-more"> <a class="text-clip-1"
                                                    href="{{ route('page', $list->slug) }}"> Read
                                                    More... </a></div>
                                        </div>
