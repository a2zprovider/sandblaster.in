        ...
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h4 class="fw-bold m-0">
                    <span class="text-muted fw-light">Blog /</span>@if($trash) Trash @endif View
                </h4>
                <div>
                    @if(!$trash)
                    @if(auth()->user()->role=='admin')
                    <div class="btn-primary btn" id="add_permissions" style="margin-right: 20px;"> Permissions </div>
                    <a href="{{ route('admin.blog.index','trash') }}" class="btn-primary btn" style="margin-right: 20px;"> Trash </a>
                    @endif
                    ...
                    @else
                    <a href="{{ route('admin.blog.index') }}" class="btn-primary btn"> Back </a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="permissionModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="{{ route('admin.author.blog') }}" method="POST">
                        @csrf
...
<script type="text/javascript">
    $(function() {
        ...
        var table = $('.data-table').dataTable({
            ...
            serverSide: true,
            searchable: true,
            ajax: "{{ route('admin.blog.index') }}" + $(location).attr("search"),
        ...
        $('select.change-user').on('change', function() {
            ...
            $.ajax({
                url: '{{ route("admin.blog.permission") }}',
                type: 'get',
                data: {
                    request: 2,
                    id: id,
                ...
                success: function(response) {
                    ...
                    html = html + '</div>';
                    $('#permissionModal').find('.blog-permissions').html(html);
                }
            });
        });
