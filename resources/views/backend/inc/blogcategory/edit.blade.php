...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        {{ Form::open(['url' => route('admin.blogcategory.update',$blogcategory->id), 'method'=>'PUT', 'files' => true, 'class' => 'needs-validation','novalidate']) }}
        <div class="card mb-4 header-sticky">
            <div class="d-flex justify-content-between align-items-center py-3 mb-2 card-body">
                <h4 class="fw-bold m-0">
                    <span class="text-muted fw-light">Blog Category /</span> Edit
                </h4>
                <div>
                    <a href="{{ route('admin.blogcategory.index') }}" class="btn-primary btn" type="reset" style="margin-right: 20px;">Cancel</a>
                    <button class="btn-primary btn" type="submit">Update</button>
                </div>
            </div>
        </div>
        
        @include('backend.inc.blogcategory._form')
...
<script>
    ...
    myDropzone.emit("thumbnail", mockFile, "{{ url('images/blogcategory/' . $blogcategory->image) }}");
    myDropzone.emit("complete", mockFile);
    $('.image_file').val('{{ $blogcategory->image }}')
</script>
@endif
