        ...
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h4 class="fw-bold m-0">
                    <span class="text-muted fw-light">Client /</span>@if($trash) Trash @endif View
                </h4>
                <div>
                    @if(!$trash)
                    @if(auth()->user()->role=='admin')
                    <a href="{{ route('admin.client.index','trash') }}" class="btn-primary btn" style="margin-right: 20px;"> Trash </a>
                    @endif
                    <div class="btn-danger btn" id="delete_record" style="margin-right: 20px;"> Delete </div>
                    <a href="{{ route('admin.client.create') }}" class="btn-primary btn text-white"> Add </a>
                    @else
                    <a href="{{ route('admin.client.index') }}" class="btn-primary btn"> Back </a>
                    @endif
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    $(function() {
        ...
        var table = $('.data-table').dataTable({
            ...
            serverSide: true,
            searchable: true,
            ajax: "{{ route('admin.client.index') }}" + $(location).attr("search"),
    ...
    function handelDelete(id) {
        var url = "{{ route('admin.client.index') }}";
        url = url + '/' + id;
        Swal.fire({
            title: "Are you sure?",
