        ...
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h4 class="fw-bold m-0">
                    <span class="text-muted fw-light">Role /</span> View
                </h4>
                <div>
                    <div class="btn-danger btn" id="delete_record" style="margin-right: 20px;"> Delete </div>
                    <a href="{{ route('admin.role.create') }}" class="btn-primary btn text-white"> Add </a>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
            ...
            if (ids_arr.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route("admin.role.deleteAll") }}',
                            type: 'post',
                            data: {
                                request: 2,
                                ids_arr: ids_arr
    ...
    function handelDelete(id) {
        var url = "{{ route('admin.role.index') }}";
        url = url + '/' + id;
        Swal.fire({
            title: "Are you sure?",
