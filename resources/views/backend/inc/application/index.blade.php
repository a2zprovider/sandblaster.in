        ...
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h4 class="fw-bold m-0">
                    <span class="text-muted fw-light">Application /</span>@if($trash) Trash @endif View
                </h4>
                <div>
                    @if(!$trash)
                    @if(auth()->user()->role=='admin')
                    <div class="btn-primary btn" id="add_permissions" style="margin-right: 20px;"> Permissions </div>
                    <a href="{{ route('admin.application.index','trash') }}" class="btn-primary btn" style="margin-right: 20px;"> Trash </a>
                    @endif
                    <div class="btn-danger btn" id="delete_record" style="margin-right: 20px;"> Delete </div>
                    <a href="{{ route('admin.application.create') }}" class="btn-primary btn text-white"> Add </a>
                    @else
                    <a href="{{ route('admin.application.index') }}" class="btn-primary btn"> Back </a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="permissionModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="{{ route('admin.author.application') }}" method="POST">
                        @csrf
                        ...
                        <div class="modal-body">
                            ...
                            <div class="application-permissions">
                                <div class="row">
                                    <div class="col mb-3">
...
<script type="text/javascript">
    $(function() {
        ...
        var table = $('.data-table').dataTable({
            ...
            serverSide: true,
            searchable: true,
            ajax: "{{ route('admin.application.index') }}" + $(location).attr("search"),
        ...
        $('select.change-user').on('change', function() {
            ...
            $.ajax({
                url: '{{ route("admin.application.permission") }}',
                type: 'get',
                data: {
                    request: 2,
                    id: id,
                ...
                success: function(response) {
                    ...
                    html = html + '</div>';
                    $('#permissionModal').find('.application-permissions').html(html);
                }
            });
        });
        // Delete record
            ...
            if (ids_arr.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route("admin.application.deleteAll") }}',
                            type: 'post',
                            data: {
                                request: 2,
                                ids_arr: ids_arr
    ...
    function handelDelete(id) {
        var url = "{{ route('admin.application.index') }}";
        url = url + '/' + id;
        Swal.fire({
            title: "Are you sure?",
