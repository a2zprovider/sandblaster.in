...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        {{ Form::open(['url' => route('admin.application.update',$application->id), 'method'=>'PUT', 'files' => true, 'class' => 'needs-validation','novalidate']) }}
        <div class="card mb-4 header-sticky">
            <div class="d-flex justify-content-between align-items-center py-3 mb-2 card-body">
                <h4 class="fw-bold m-0">
                    <span class="text-muted fw-light">Application /</span> Edit
                </h4>
                <div>
                    <a href="{{ route('admin.application.index') }}" class="btn-primary btn" type="reset" style="margin-right: 20px;">Cancel</a>
                    <button class="btn-primary btn" type="submit">Update</button>
                </div>
            </div>
        </div>
        
        @include('backend.inc.application._form')
...
<script>
    ...
    myDropzone.emit("thumbnail", mockFile, "{{ url('images/application/' . $application->image) }}");
    myDropzone.emit("complete", mockFile);
    $('.image_file').val('{{ $application->image }}')
</script>
@endif
@if($application->thumb_image)
<script>
    var mockFile = {
        name: "{{ $application->thumb_image }}",
        size: "2"
    };
    myDropzone2.emit("addedfile", mockFile);
    myDropzone2.emit("thumbnail", mockFile, "{{ url('images/application/' . $application->thumb_image) }}");
    myDropzone2.emit("complete", mockFile);
    $('.thumb_image_file').val('{{ $application->thumb_image }}')
</script>
@endif
